...
public class GamePanel extends JPanel implements ActionListener{
	...
	public void draw(Graphics g) {
		if(running) {
			...
			for (int i = 0; i < SCREEN_HEIGHT/UNIT_SIZE; i++) {
				g.drawLine(i*UNIT_SIZE, 0, i*UNIT_SIZE, SCREEN_HEIGHT); // y Lines
				g.drawLine(0, i*UNIT_SIZE, SCREEN_WIDTH, i*UNIT_SIZE); // x Lines
			}
			...
			for(int i = 0; i<bodyParts; i++) {
				if(i==0) { 
					...
					g.setColor(new Color( // (optional) change (random) body color
						random.nextInt(255), 
						random.nextInt(255), 
						random.nextInt(255)
					));
	...
	private void newApple() {
		// gen coordinates for a new apple
	...
	public void move() {
		/**
		 *  TODO: 
		 *  CREATE LEVELS (TIMER), Lv1, Lv2 ...LvN
		 *  INCREASE SPEED PER LEVEL - USE Timer 2x, 3x
		 *  ANIMATE COLLISION - DISPLAY MULTI-COLOR EXPLOSION
		 *  create leader board
		 *  previous score
		 *  best score
		 */
